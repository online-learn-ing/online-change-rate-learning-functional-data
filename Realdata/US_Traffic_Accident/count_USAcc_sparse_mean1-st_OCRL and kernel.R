...
### mean
{
  ...
  # update
    ...
    file_path <- paste0(year, "_sparse_count_US_Accidents_fdda.Rdata")
    load(file_path)
    Kmax <- length(data_sparse) + start
    for(K in (start+1):Kmax){
      ...
      # theta
      {
        ...
        theta_mu <-  mean(res_theta_mu$P[1,1,11:90 ,1] * mu_thi_deri[11:90]^2)
      }
...
### mean
{
  ...
  save(mus1, h1, time,sigma_mu1,theta_mu1, file="./sparse_count_USAcc_res_mean1_batch.Rdata")
}
