...
### mean
{
  ...
  # update
    for(year in 2018:2022){#data each year 
    file_path <- paste0( year, "_dense_count_US_Accidents_fdda.Rdata")
    load(file_path)
    Kmax <- length(data_dense) + start
    for(K in (start+1):Kmax){
      ...
      # theta
      {
        ...
        theta_mu <- (N-NK)/N * theta_mu + NK/N*(mean(res_theta_mu$P[1,1,6:95 ,1] * mu_thi_deri[6:95]^2)) 
      ...
      # sigma
      {
        ...
        # sigma_mu <- (N-NK)/N * sigma_mu + NK/N * (mean(r) + mean((r-sigma_eps)*den)/0.6)#1.65
        h_sigma_mu1 <- G * N^(-1/7)
...
### mean
{
  ...
  save(mus1, h1, time,sigma,sigma_mu1,theta_mu1, file="./dense_count_USAcc_res_mean1_batch.Rdata")
}
